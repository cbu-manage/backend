    ...
    @Operation(
            ...
            description = """
                    ...
                    **권한별 조회 범위**
                    ...
                    - 그 외: 본인이 ACTIVE 상태인 그룹의 보고서 + 본인이 작성한 보고서
                      (소속 그룹이 없어도 본인이 쓴 보고서는 조회된다)

                    **groupIds 필터**
                    - 여러 그룹 ID를 동시에 전달하면 해당 그룹들 중 하나에 속하는 보고서를 조회합니다 (OR).
                    - ADMIN / 회장 / 부회장 / 서기: 전달한 groupIds로 필터링.
                    - 그 외: 본인의 ACTIVE 그룹과 전달한 groupIds의 교집합으로 필터링(본인이 작성한 보고서는 함께 조회).

                    **groupCategory 필터**
                    - 카테고리로 좁혀 볼 때는 본인 작성 보고서를 따로 끌어오지 않는다. 지정한 카테고리 안에서만 조회한다.
